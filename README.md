...
主动碰撞层会在检测到碰撞后，触发自身的回调（`OnTriggerEnter` / `OnTriggerStay` / `OnTriggerExit`）。在两个相互碰撞的层级之间，你应该选择数量较少的那一个作为主动碰撞层，以减少计算量。

例如，在一个弹幕地狱游戏中，有很多子弹和较少的敌人，那么你应该选择敌人所在的层级作为主动碰撞层。反之，如果有很多敌人，而子弹较少，那么你应该选择子弹所在的层级作为主动碰撞层。

### 被动碰撞层

被动碰撞层不会触发自身回调，但是当它被其他主动碰撞层碰撞时，会触发其他主动碰撞层的回调。
...
碰撞器通过 Action 回调触发碰撞事件。可订阅以下回调：
`OnTriggerEnter`：进入碰撞时触发一次。
`OnTriggerStay`：持续碰撞时每帧触发。
`OnTriggerExit`：离开碰撞时触发一次。
注意：这些回调仅对“主动碰撞层”触发，被动层不会触发自身回调。
